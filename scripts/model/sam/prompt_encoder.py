...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        self.sparse_embeddings = nn.Embedding(1, embed_dim)
    ...
    def forward(
        ...
        # sparse_embeddings = self.sparse_embeddings.weight.unsqueeze(dim =0).expand(bs, -1, self.embed_dim)
        sparse_embeddings = torch.empty(
            (bs, 0, self.embed_dim), dtype=text_embedding.dtype, device=self._get_device()
        )
        if points is not None:
