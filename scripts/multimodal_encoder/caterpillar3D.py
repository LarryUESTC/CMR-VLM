...
class sparseMLP_BNAct(nn.Module):
    ...
    def __init__(self, W, H, D, channels):
        super().__init__()
        assert W == H
        self.channels = channels
...
class CaterpillarBlock(nn.Module):
    ...
    def __init__(self, W, H, D, channels, step, drop_out, drop_path):
        super().__init__()
        self.localMix = ShiftedPillarsConcatentation_BNAct(channels, step)
        self.globalMix = sparseMLP_BNAct(W, H, D, channels)
...
class Caterpillar3D(nn.Module):
    def __init__(self, img_size=[32, 256, 256], in_chans=1, patch_size=[4, 16, 16], embed_dim=[96,192,384,768], depth=[2,6,14,2],
        ...
        for i in range(len(depth)):
            ...
            for j in range(depth[i]):
                # drop_path=dpr[j+shift]
                self.blocks.append(nn.Sequential(CaterpillarBlock(grid_size[1], grid_size[2], grid_size[0],
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        x_cls = x.mean(dim=[2,3,4]).flatten(1)
        B, C, D, H, W = x.shape
        out_shape = (B, C, D * H * W)
        x = x.reshape(*out_shape)
        return torch.cat((x_cls.unsqueeze(-1), x), dim=2).permute(0,2,1).contiguous(), [x.permute(0,2,1).contiguous()]
...
class Cater3DTower(nn.Module):
    def __init__(self, config):
        ...
        self.vision_tower = Caterpillar3D(
            img_size=[32, 256, 256],
            patch_size=[2, 4, 4],
            embed_dim=[40, 80, 160, 768],
            depth=[2, 6, 10, 2],
            down_stride=[4, 2, 2, 2],
            shift_step=[1, 1, 1, 1]
        )
